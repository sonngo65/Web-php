	...
	<div class="content">
		<div class="login_panel">
			...
			<form action="<?= BASE_URL . "login/checkLogin" ?>" method="post" id="member">
				...
				<?php if (session::get('login_status') != false) { ?>
					<div><span style="font-size:14px;color:red;padding:10px"><?= session::get('login_status') ?></span></div>
		...
		<div class="register_account">
			...
			<form method="POST" action="<?= BASE_URL . 'login/register' ?>">
				...
				<?php if (session::get('register_status') != false) { ?>
					<div><span style="font-size:14px;color:red;"><?= session::get('register_status') ?></span></div>
				<?php session::remove('register_status');
				} ?>
