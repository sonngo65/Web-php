		...
		<div class="section group">
			<?php foreach ($getData_byProduct_type as  $valueProduct_data) { ?>
				<div class="grid_1_of_4 images_1_of_4">
					...
					<a href="<?= BASE_URL . "productDetail/index/" . $valueProduct_data['id'] ?>"><h2 style="font-weight: 900;"><?= $valueProduct_data['name'] ?></h2></a>
		...
		<ul class="Pagination">
			<?php for ($num = 1; $num <= $number_page[0]; $num++) { ?>
				<?php if ($num != $number_page[2]) { ?>
					<li class="page-item"><a class="page-link" href="<?= BASE_URL . "product/" ?>getByCategory/<?= $category_id ?>/<?= $number_page[1] ?>/<?= $num ?>"><?= $num ?></a></li>
				<?php } else { ?>
